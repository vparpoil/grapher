...
describe('Reactive count tests', function () {
  before(async () => {
    await Meteor.callAsync('resetPosts');
  });
  ...
  it('Should update when a document is added', function (done) {
    ...
    Tracker.autorun(async (c) => {
      if (handle.ready()) {
        ...
        const newId = await Meteor.callAsync('addPost', 'text4');
        const newCount = query.getCount();
        assert.equal(newCount, 4);

        await Meteor.callAsync('removePost', newId);
        handle.stop();
  ...
  it('Should update when a document is removed', function (done) {
    ...
    Tracker.autorun(async (c) => {
      if (handle.ready()) {
        ...
        await Meteor.callAsync('removePost', 'removeid');
